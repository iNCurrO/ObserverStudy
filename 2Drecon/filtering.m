...
uu = ((-param.nu/2+0.5):1:(param.nu/2-0.5))*param.du + param.off_u;

w = (param.DSD)./sqrt((param.DSD)^2+uu.^2);
...
filt_len = max(64,2^nextpow2(param.nu));
[ramp_kernel] = ramp_flat(filt_len);

d = 1; % cut off (0~1)
[filt] = Filter(param.filter, ramp_kernel, filt_len, d);
% filt = repmat(filt',[1 param.nu]);

for i=1:param.nProj
    
    fproj = (zeros(filt_len,1,'single'));
